...
def predict():
	...
	inp = np.array([[int(Age), int(Gender), int(Interest), int(Impressions), int(Click), int(Spent)]])
...
def LinearProgramming(model, scaler, constraint):
    ...
    if constraint['ClickMax']:
    	...
    	ct_max[0][-3]=min(float(constraint['ClickMax']), float(constraint['SpentMax'])*0.8)
    if constraint['ImpressionsMax']:
    	ct_max[0][-4]=min(float(constraint['ImpressionsMax']), float(constraint['SpentMax'])*490)
    print(ct_max)
    print(ct_min)
    # Add constraints
